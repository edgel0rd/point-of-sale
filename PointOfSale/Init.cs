...
namespace PointOfSale
{
    public class Init
    {
        ...
        public void CreateDatabase()
        {
            ...
            //employee
            exec($"CREATE TABLE IF NOT EXISTS employee(id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, name VARCHAR(128) NOT NULL, role ENUM('Administrator','Manager','Cashier') NOT NULL, email VARCHAR(128) NOT NULL UNIQUE KEY, password VARCHAR(128) NOT NULL, status ENUM('Active','Standby','Terminated') NOT NULL DEFAULT 'Active', {add});");
            ...
            //TRANSACTIONS
            exec($"CREATE TABLE IF NOT EXISTS transaction(id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, customer_id INT NULL, employee_id INT NOT NULL, status ENUM('Void','Valid') NOT NULL DEFAULT 'Valid', {add}, FOREIGN KEY(customer_id) REFERENCES employee(id));");
